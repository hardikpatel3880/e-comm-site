...
<form method="POST" action="{{ route('productSizeMaster.update', $sizeMaster->id) }}">
    @csrf
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>Size:</strong>
                <input type="text" name="size" placeholder="Size" id="size" class="form-control"
                    value="{{ $sizeMaster->size }}">
            </div>
        </div>

        <div class="col-sm-12 col-lg-10 col-md-12 mt-2">
            <div class="form-check form-check-inline">
                <input type="checkbox" name="status" {{ $sizeMaster->status == 'Y' ? 'checked' : '' }}>
